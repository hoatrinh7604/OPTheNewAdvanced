...
public class LevelSelectionLogic : MonoBehaviour {
	...
	void Awake()
	{
		...
		foreach (LevelList level in levelList)
		{
			...
			{
				//Debug.Log ("level "+ level.LevelIndex + " is finished!");
			...
			if(PlayerPrefs.HasKey("startsCount" + level.LevelIndex.ToString()))
			{
				...
				//Debug.Log ("star = "+ level.starsCount);
			}
		}
	...
	void Update () 
	{
		...
		if(Input.GetMouseButtonUp(0))
		{
			for (int i = 0; i < levelList.Count; i++)
			{
				...
				{
					currentLevelIndex = levelList [i].LevelIndex;//Me: del
					SceneManager.LoadScene(levelList[i].currentLevelIndexMap);//Me: SceneManager.LoadScene(levelList [i].LevelIndex);
